...
<body>
    ...
    <div class="sidebar">
        ...
        <li><a href="http://127.0.0.1:5000/"><i class='bx bx-shield-quarter'></i>Dark Web Report</a></li>
    ...
    <div class="content">
        ...
        <main>
            <div class="header">
                <div class="left">
                    <h1>GitHub Username Checker</h1>
                    <p>Check if a GitHub username is available or not</p>
                </div>
            </div>
    ...
    <script>
        document.getElementById('usernameForm').addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (response.ok) {
                    resultElement.textContent = `GitHub username '${username}' exists!`;
                    resultElement.classList.add('exists');
                    profileLink.innerHTML = `<a href="https://github.com/${username}" target="_blank">Visit Profile</a>`;
                } else if (response.status === 404) {
                    resultElement.textContent = `GitHub username '${username}' does not exist`;
                    resultElement.classList.remove('exists');
                    profileLink.innerHTML = '';
                } else {
                    resultElement.textContent = 'Error checking username. Please try again later.';
                    resultElement.classList.remove('exists');
                    profileLink.innerHTML = '';
                }
            } catch (error) {
