...
<div class="sidebar">
    ...
    <script>
        ...
        form.addEventListener('submit', async function (event) {
            ...
            const apiUrl = `https://dns-history.whoisxmlapi.com/api/v1?apiKey=${apiKey}&ip=${ipInput}`;
            ...
            try {
                ...
                data.result.forEach((item, index) => {
                    ...
                    resultItem.innerHTML = `
                        <p><strong>Domain Name:</strong> ${item.name}</p>
                        <p><strong>First Seen:</strong> ${formatDate(item.first_seen)}</p>
                        <p><strong>Last Visit:</strong> ${formatDate(item.last_visit)}</p>
                    `;
